...
// Generate DBML
function generateDbml(): string {
  let dbml = "// Database schema for Job Marketplace\n\n"

  // Add tables
  models.forEach((model) => {
    dbml += `Table ${model.name} {\n`
    dbml += "  _id ObjectId [pk]\n"

    model.fields.forEach((field) => {
      const requiredSymbol = field.required ? "not null" : ""
      dbml += `  ${field.name} ${field.type} ${requiredSymbol}\n`
    })

    dbml += "  createdAt DateTime\n"
    dbml += "  updatedAt DateTime\n"
    dbml += "}\n\n"
  })
  ...
  relationships.forEach((rel) => {
    ...
    dbml += `Ref: ${rel.source}._id ${relationshipType} ${rel.target}.${rel.sourceField}\n`
  })

  return dbml
}

// Main function to generate and save the DBML
function generateDiagram() {
  const dbml = generateDbml()
  const outputPath = path.join(process.cwd(), "job-marketplace-schema.dbml")
